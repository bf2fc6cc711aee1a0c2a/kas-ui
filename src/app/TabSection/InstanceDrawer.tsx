...
const InstanceDrawer: React.FunctionComponent<InstanceDrawerProps> = ({
  ...
  const [kafkaInstance, setKafkaInstance] = useState();
  ...
  const fetchKafkas = async () => {
    await apisService.getKafkaById('').then((res) => {
  ...
  */
  ...
  const handleTab1Click = (event, tabIndex) => {
    setActiveTab1Key(tabIndex);
  };

  const handleTab2Click = (event, tabIndex) => {
  ...
  const detailsTab = (
    <>
      ...
      <Grid>
        ...
        {renderGridItemDetail('ID', '1iSY6RQ3JKI8Q0OTmjQFd3ocFRg')}
        {renderGridItemDetail('Owner', 'US East, N. Virginia')}
        {renderGridItemDetail('Created', '[date created]')}
        {renderGridItemDetail('Updated', '[date updated]')}
      </Grid>
    </>
  );
